...
from cvgeomkit.types import ArrayLike, Numeric
...
class Line(Hashable):
    ...
    def intersection(self, another_line: Self, image: ArrayLike) -> Intersection | None:
        """
        ...
        Args:
            ...
            image (ArrayLike): The image used to check if the intersection point lies within its bounds.
    ...
    def limit_to_img(self, img: ArrayLike) -> tuple[Point, Point]:
        """
        Returns two endpoints of the line segment clipped to the image boundaries.

        Args:
            img (ArrayLike): The image array used to determine dimensions.

        Returns:
            tuple[Point, Point]: Two (x, y) points that define the visible
    ...
    def to_segment(self, image: ArrayLike) -> "LineSegment":
    ...
    def from_hough_segment(cls, hough_segment: tuple[int, int, int, int]) -> Self:
        ...
        Args:
            hough_segment (tuple[int, int, int, int]): A 4-tuple (x1, y1, x2, y2) representing the endpoints of the line.
    ...
    def from_points(cls, p1: Point, p2: Point) -> Self:
        ...
        Args:
            p1 (Point): The first point (x1, y1).
            p2 (Point): The second point (x2, y2).
